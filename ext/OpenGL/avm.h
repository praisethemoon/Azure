...
#define AZ_BLOCKHEADERSIZE ((long)sizeof(mblock)+(long)sizeof(objheader))
#define AZ_TABLESIZE(num) ((long)(num*sizeof(cnode)+sizeof(long)*2))
#define AZ_INITIALTOTSIZE 100
...
#define ishostfunction(name) (az_getfunc(name)!=0)
...
#define getdata(heap, offset) (heap+offset+AZ_BLOCKHEADERSIZE)
...
typedef enum
{
	...
	opDivide, opMod, opEqual, opNeq, opGreater, opGeq, opLess, opLeq,
...
struct _symbol* getinsfunc(struct _context* ctx, unsigned int ins);
