		  ...
		  int op, operand* op1, operand* op2, operand* opdest)
{
	...
	if (nextslot >= casm->size)	/* buffer is full, reallocate */
	{
		...
		if ((casm->instructions =
			...
			setlasterror(ctx, AZ_OUT_OF_MEMORY, "Not enough memory.\n");
	}
...
void transquad(context* ctx, assembly* casm, int quadnum)
{
	...
	case opAssign:
		...
		break;
	case opNeg:			case opNot:			case opBitcom:
		loadreg(ctx, casm, &quad->op1, ra, local, temp);
		o.ival = ra;
	...
	case opLess:		case opLeq:			case opConcat:
		loadreg(ctx, casm, &quad->op1, ra, local, temp);
...
void translate(context* ctx, assembly* casm)
{
	...
	for (i=0; i<ctx->funcs.nextslot; i++)
	{
			...
			if (!ishostfunction(ctx->funcs.funcs[i].callee))
			{
				...
				casm->instructions[k].op1.ival = (int)az_getfunc(ctx->funcs.funcs[i].callee);
	}
	/*
	for (i=0; i<asm->size; i++)
	{
		if (asm->instructions[i].op == opLFUNC)
		{
			s = getinsfunc(ctx, i);
			asm->instructions[i].op1.ival = 32;
		}
	}
	*/
	...
	casm->stacksize = AZ_DEFAULT_STACK_SIZE;
