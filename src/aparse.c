...
{
	return getlasterror(ctx)!=AZ_NO_ERROR?
...
void parseerror(LexInfo* li, context* ctx)
{
	...
	if (li->lineerrors[li->line_num] == 0)
	{
		...
		if (!token_name)	/* general error */
			setlasterror(findcontextroot(ctx), AZ_PARSER_SYNTAX_ERROR,
		"%s: syntax error, line %d\n", li->source, li->line_num);
		else
			setlasterror(findcontextroot(ctx), AZ_PARSER_SYNTAX_ERROR,
		"%s: %s expected, line %d\n", li->source, token_name, li->line_num);
...
tree_node* makenode(context* ctx, int token, tree_node* left, tree_node* right, tree_node* sibling)
{
	...
	if (t == NULL)
	{
		setlasterror(ctx, AZ_OUT_OF_MEMORY, "Out of memory.\n");
		return NULL;
	}
	t->isleaf = 0;
	t->op = token;
	...
	case IDENTIFIER:
		...
		if (li->next_token.token == '=')	/* a possible declaration */
			if (!lookupall(li, ctx, li->token.dbuf))
			{
				...
				sprintf(msg, "%s: implicit variable declaration, line %d\n", li->source, li->line_num);
				warning(msg);
			}
		if (!(s=lookupall(li, ctx, li->token.dbuf)) && !ishostfunction(li->token.dbuf))
			setlasterror(findcontextroot(ctx), AZ_PARSER_UNDEFINED,
			"%s: identifier \"%s\" undefined, line %d.\n", li->source, li->token.dbuf, li->line_num);
		t = makeleaf(ctx, li->token);
		if (s) sym2node(s, t);
...
void checkargs(LexInfo* li, context* ctx, const char* funcname, tree_node* args)
{
	...
	if (!s->paramsvariable&&s->numnamedparams!=numofsiblings(args) ||
		...
		setlasterror(ctx, AZ_PARSER_DIFFERENT_NUMOFARGS,
			"%s: number of arguments is different from definition, line %d\n",
			li->source, li->line_num);
}
...
void qbuild(context* ctx, intermediate** pp, eOpcode op, qoperand* op1, qoperand* op2, qoperand* opdest)
{
	...
	if (nextslot >= p->size)	/* buffer is full */
	{
		...
		if ((p->quadruples=realloc(p->quadruples, p->size*sizeof(quadruple))) == NULL)
			setlasterror(ctx, AZ_OUT_OF_MEMORY, "Not enought memory.\n");
	}
...
void addtag(context* ctx, statementinfo* si, char type, long slot, tree_node* expr)
{
	/* check bounds here */
...
void addcall(context* ctx, long slot, const char* caller, const char* callee)
{
...
void patchcalls(LexInfo* li, context* ctx)
{
	...
	for (i=0; i<ctx->calls.nextslot; i++)
	{
		...
		if (s == NULL)
			setlasterror(ctx, AZ_PARSER_UNKNOWN_ERROR, "Unknown error occured\n.");
		else
...
void addfunc(context* ctx, long slot, char operand, const char* caller, const char* callee)
{
...
qoperand* genfunc(context* ctx, tree_node* t, intermediate** pp)
{
	...
	if (t->type == IDFUNCTION)	/* direct call */
	{
		...
		if (ishostfunction(t->token.dbuf))
		{
			qbuildbranch(ctx, pp, opCallhost, NULL, (int)az_getfunc(t->token.dbuf));
...
/* DEF funcname "(" paramlist ")" compound */
void dofunction(LexInfo* li, context* pctx, intermediate** pp)
{
	...
	if (pctx->compoundlevel != 0)
		setlasterror(pctx, AZ_PARSER_NO_LOCAL_FUNCTION,
	...
	if (li->next_token.token == '{')		/* a definition */
	{
		...
		setlasterror(pctx, AZ_PARSER_SYNTAX_ERROR,
		"Syntax error, line %d.\n", li->line_num);
	freenode(fn);
}
...
/*
	...
	Implementation:
	The FOREACH statement generates the following quadruples:
...
void doforeach(LexInfo* li, context* ctx, intermediate** pp, statementinfo* si)
{
	...
	if (li->next_token.token == ',')		/* is it key/value? */
	{
		...
		t1 = t;
	else setlasterror(findcontextroot(ctx), AZ_PARSER_SYNTAX_ERROR,
		"%s: syntax error, line %d\n", ctx->source, li->line_num);
	match(li, ctx, _IN);
	t2 = exprlist(li, ctx);
	match(li, ctx, ')');
...
void dodo(LexInfo* li, context* ctx, intermediate** pp, statementinfo* si)
{
	qoperand* o;
	int slot;
	clStep(li);
...
void dovar(LexInfo* li, context* pctx, intermediate** pp)
{
	...
	while (1)
	{
		if (lookup(pctx, &pctx->sbt, li->next_token.dbuf))
			setlasterror(pctx, AZ_PARSER_REDEFINED,
			"Identifier redefinition in line %d.\n ", li->line_num);
...
void docompound(LexInfo* li, context* pctx, intermediate** pp, symboltable* sbtp, int isfuncbody, tree_node* fn, statementinfo* si, int pls, int pts)
{
	...
	if (isfuncbody)
	{
		...
		{
			setlasterror(pctx, AZ_OUT_OF_MEMORY, "Out of memory.\n");
			return;
		}
		p->size = 1024;
		...
		p = *pp;
	match(li, &ctx, '{');
	if (li->next_token.token == '}')	/* empty compound statement */
		clStep(li);
	else
	{
		while (li->next_token.token != ENDOFFILE && li->next_token.token != '}' && validatecontext(&ctx))
		{
...
{
	char *buf, path[AZ_MAX_PATH_LEN*2]={0};
	clStep(li);
	getdir(ctx->source, path);
	strcat(path, li->next_token.dbuf);
	if (!(buf=readfile(path)))
	{
		setlasterror(ctx, AZ_PARSER_INCLUDE_NOT_FOUND, "Compile error: include file \"%s\" not found\n", path);
		return;
	}
	parse(path, buf, ctx);
	clStep(li);
...
void dostatement(LexInfo* li, context* ctx, intermediate** pp, statementinfo* si)
{
	...
	case '{': docompound(li, ctx, pp, ctx->sbtp, 0, NULL, si, ctx->sbt.offset, ctx->temp); break;
	case WHILE: dowhile(li, ctx, pp, si); break;
	case DO: dodo(li, ctx, pp, si); break;
