...
{
	hp->size = hp->availsize = AZ_DEFAULT_HEAP_SIZE;
	if ((hp->heap=(char*)calloc(hp->size,sizeof(char))) == NULL)
	{
		setvmerror(vm, AZ_VM_OUT_OF_MEMORY);
		return;
	}
...
void run(context* ctx, avm* vm)
{
	...
	if (vm->numins==0)
		return;
	while (res==AZ_NO_ERROR && vm->lasterror==AZ_NO_ERROR)
	{
		res = step(ctx, vm);
...
{
	word w;
	long soff = newstring(vm, (char*)sval);
	sets(&w, soff);
	setglobal(vm, idname, &w);
}
...
void push(avm* vm, stack* s, word* w)
{
	if (vm->regs[rs].entity.ival < s->size)
		...
		setvmerror(vm, AZ_VM_STACK_OVERFLOW);
}
...
word* pop(avm* vm, stack* s)
{
	...
	setvmerror(vm, AZ_VM_STACK_UNDERFLOW);
	return NULL;
}
...
void ir(avm* vm, int breg, int ireg, int dreg)
{
	...
	{
		setvmerror(vm, AZ_VM_INVALID_TABLE);
		return;
	}
...
{
	int m, iop1, iop2, iopdest, i, res = AZ_NO_ERROR;
	...
	case opBitcom:
		...
		else setvmerror(vm, AZ_VM_INVALID_OPERATION);
		break;
	case opBitand:
	...
	case opCall: case opCallhost:	case opCallin:
		if (ins->op==opCallin)	/* note that the lookup is very slow! */
		{
			...
			else
			{
				...
				if (!s)
					setvmerror(vm, AZ_VM_CONTEXT_CORRUPTED);
				else
		...
		if (vm->regs[rs].entity.ival+framesize>vm->cs->size-1)
			res = AZ_VM_STACK_OVERFLOW;
#ifndef AZ_ENABLE_OPTIMIZATION
		...
		case opCallhost:
			...
			if (az_getfunc("resume") != *(userfunc)ins->opdest.ival)
			{
				rethost(vm);
				vm->pc++;
			}
	...
	case opNewtable: sett(&vm->regs[ins->opdest.ival],
			TOTinsert(vm, newtable(vm,AZ_INITIALTABLESIZE))); break;
	...
	case opJnz:
		...
		vm->pc = iopdest;
		return AZ_NO_ERROR;
	case opJmp: vm->pc = iopdest; return AZ_NO_ERROR;
...
int getint(avm* vm, int ind)
{
	...
	if (gettypew(w) != TINTEGER)
		setvmerror(vm, AZ_VM_INVALID_ARGUMENT);
...
int getglobali(avm* vm, const char* idname)
{
	...
	if (gettypew(w) != TINTEGER)
		setvmerror(vm, AZ_VM_INCORRECT_TYPE);
	return w->entity.ival;
}
...
{
	word w;
	long soff = newstring(vm, (char*)str);
	sets(&w, soff);
	push(vm, vm->cs, &w);
}
